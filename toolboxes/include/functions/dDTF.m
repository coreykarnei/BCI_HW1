function [gamma2,H,S,zeta] = dDTF(ts,low_freq,high_freq,p,fs)
...
for k = 1:nfre
	...
	for i = 1:nchan
        for j = 1:nchan
            Zf(i,j) = Cf(i,j)/sqrt(Cf(i,i))/sqrt(Cf(j,j));
        end
    end
...
% denominator = sqrt(sum(sum(sum((abs(Z).^2).*(abs(H).^2)))));
% zeta = abs(Z).*abs(H)/denominator;
zeta = Z;