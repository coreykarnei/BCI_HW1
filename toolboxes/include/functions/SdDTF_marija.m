function [gamma2,H,S,zeta,C] = SdDTF_marija(ts,low_freq,high_freq,p,fs)
...
for k = 1:nfre
    ...
    if dett2 == 0
        ...
        for i = 1:nchan
            for j = 1:nchan
                ...
                Apf(j,:) = [];
                det2 = det(Apf);
                det2 = abs(det2);%det2.*conj(det2);
                theta2(i,j,k) = det2/dett2;
                theta2(i,j,k) = theta2(i,j,k)^2;
	...
	for i = 1:nchan
        for j = 1:nchan
            Cn(i,j) = Cf(i,j)/sqrt(Cf(i,i))/sqrt(Cf(j,j));
        end
    end
...
denominator = sqrt(sum(sum(sum((abs(C).^2).*(abs(H).^2)))));
zeta = abs(C).*abs(H)/denominator;
% zeta = Z;